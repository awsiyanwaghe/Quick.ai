...
export const generateImage = async (req, res) => {
    try {
        ...
        const { secure_url } = await clodinary.uploader.upload(base64Image)
...
export const removeImageBackground = async (req, res) => {
    try {
        ...
        const { secure_url } = await clodinary.uploader.upload(image.path, {
            transformation: [
                {
...
export const removeImageObject = async (req, res) => {
    try {
        ...
        const { public_id } = await clodinary.uploader.upload(image.path,)

        const imageUrl = clodinary.url(public_id, {
